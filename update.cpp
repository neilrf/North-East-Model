...
void update(class system &sys, class simulation &sim, int i){
	...
	if (!(isnan(flip_prob))){ //to avoid rogue 1's returning after all sites have updates in q=1 case
		...
		if (u <= flip_prob){
			...
			for (i=0; i<site_ptr->num_inf; i++){
				if (site_ptr->influence[i] != nullptr){
					bool mobile_test = true;
					for (int j=0; j < site_ptr->num_con; j++){
						if (site_ptr->influence[i]->constraint[j]->spin != 0){
							mobile_test = false;
							break;
						}
					}
